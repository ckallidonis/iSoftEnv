#/bin/bash

BUILD_DIST=Intel_19.0.4_CPU


# Package distribution names
CHROMA_DIST=chroma
CHROMA_GENPROP_DIST=chroma-devel-genprop
QMP_DIST=qmp
QDP_DIST=qdpxx
QDP3D_DIST=qdpxx_3d
LIBXML2_DIST=libxml2-2.9.9
QPHIX_DIST=qphix
MGPROTO_DIST=mg_proto
EIGEN_DIST=eigen-3.3.7
HADRON_DIST=hadron
TENSOR_DIST=tensor
ADAT_DIST=adat
HAROM_DIST=harom
COLORVEC_DIST=colorvec
REDSTAR_DIST=redstar
ADAT_PDF_DIST=adat-devel-pdf
COLORVEC_PDF_DIST=colorvec-devel-pdf
REDSTAR_PDF_DIST=redstar-devel-pdf

# Generic directories
TOPDIR=/gpfswork/rech/ual/uie52up/software
SRC_TOP=${TOPDIR}/src_CPU
INSTALL_TOP=${TOPDIR}/install/${BUILD_DIST}


# QMP directories
QMP_SRC=${SRC_TOP}/${QMP_DIST}
QMP_INSTALL=${INSTALL_TOP}/${QMP_DIST}

# QDP directories
QDP_SRC=${SRC_TOP}/${QDP_DIST}
QDP_INSTALL=${INSTALL_TOP}/${QDP_DIST}

# QDP-3D directories (Source code is same as QDP, installation is different)
QDP3D_SRC=${SRC_TOP}/${QDP_DIST}
QDP3D_INSTALL=${INSTALL_TOP}/${QDP3D_DIST}

# libxml2 directories
LIBXML2_SRC=${SRC_TOP}/${LIBXML2_DIST}
LIBXML2_INSTALL=${INSTALL_TOP}/${LIBXML2_DIST}

# Eigen directories
EIGEN_SRC=${SRC_TOP}/${EIGEN_DIST}

# QPHIX directories
QPHIX_SRC=${SRC_TOP}/${QPHIX_DIST}
QPHIX_INSTALL=${INSTALL_TOP}/${QPHIX_DIST}

# MG_PROTO directories
MGPROTO_SRC=${SRC_TOP}/${MGPROTO_DIST}
MGPROTO_INSTALL=${INSTALL_TOP}/${MGPROTO_DIST}

# HADRON directories
HADRON_SRC=${SRC_TOP}/${HADRON_DIST}
HADRON_INSTALL=${INSTALL_TOP}/${HADRON_DIST}

# TENSOR directories
TENSOR_SRC=${SRC_TOP}/${TENSOR_DIST}
TENSOR_INSTALL=${INSTALL_TOP}/${TENSOR_DIST}

# CHROMA directories
CHROMA_SRC=${SRC_TOP}/${CHROMA_DIST}
CHROMA_INSTALL=${INSTALL_TOP}/${CHROMA_DIST}

# CHROMA_GENPROP directories
CHROMA_GENPROP_SRC=${SRC_TOP}/${CHROMA_GENPROP_DIST}
CHROMA_GENPROP_INSTALL=${INSTALL_TOP}/${CHROMA_GENPROP_DIST}

# ADAT directories
ADAT_SRC=${SRC_TOP}/${ADAT_DIST}
ADAT_INSTALL=${INSTALL_TOP}/${ADAT_DIST}

# ADAT-PDF directories
ADAT_PDF_SRC=${SRC_TOP}/${ADAT_PDF_DIST}
ADAT_PDF_INSTALL=${INSTALL_TOP}/${ADAT_PDF_DIST}

# HAROM directories
HAROM_SRC=${SRC_TOP}/${HAROM_DIST}
HAROM_INSTALL=${INSTALL_TOP}/${HAROM_DIST}

# COLORVEC directories
COLORVEC_SRC=${SRC_TOP}/${COLORVEC_DIST}
COLORVEC_INSTALL=${INSTALL_TOP}/${COLORVEC_DIST}

# REDSTAR directories
REDSTAR_SRC=${SRC_TOP}/${REDSTAR_DIST}
REDSTAR_INSTALL=${INSTALL_TOP}/${REDSTAR_DIST}

# COLORVEC-PDF directories
COLORVEC_PDF_SRC=${SRC_TOP}/${COLORVEC_PDF_DIST}
COLORVEC_PDF_INSTALL=${INSTALL_TOP}/${COLORVEC_PDF_DIST}

# REDSTAR-PDF directories
REDSTAR_PDF_SRC=${SRC_TOP}/${REDSTAR_PDF_DIST}
REDSTAR_PDF_INSTALL=${INSTALL_TOP}/${REDSTAR_PDF_DIST}


# Build files directory
BUILDDIR=${INSTALL_TOP}/build_files
mkdir -p ${BUILDDIR}


export DIR_SETUP=1
